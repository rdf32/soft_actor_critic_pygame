...
targets = validate_targets(background, generate_groups(dimensions, config), dimensions, config)
state, done = draw_targets(background, targets)[1], False
...
class SACT():
    ...
    def interaction_step(self, state, targets, train=True):
        ...
        return Experience(state, action, *self.sac.interation_func(action, targets))
    ...
    def train(self):
        ...
        self.policy_model = self.sac.policy_model_fn()
        self.target_value_model_a = self.sac.value_model_fn()
        self.online_value_model_a = self.sac.value_model_fn()
        self.target_value_model_b = self.sac.value_model_fn()
        self.online_value_model_b = self.sac.value_model_fn()
        self.update_vnetworks(tau=1.0)

        self.policy_optimizer = self.policy_optimizer_fn(self.policy_model, self.sac.params['policy_lr'])
        self.value_optimizer_a = self.value_optimizer_fn(self.online_value_model_a, self.sac.params['value_lr'])
        self.value_optimizer_b = self.value_optimizer_fn(self.online_value_model_b, self.sac.params['value_lr'])
        ...
        for episode in range(1, self.params['max_episodes'] + 1):
            ...
            for _ in count():
                ...
                if np.sum(self.episode_timestep) % self.sac.params['tupdate'] == 0:
...
##########################################################################################################################################3
            ...
            episode_results.append({"mean_10_reward": mean_10_reward, "std_10_reward": std_10_reward, "mean_100_reward": mean_100_reward,
                              ...
                              "lst_100_exp_rat": lst_100_exp_rat, "mean_100_exp_rat": mean_100_exp_rat, "std_100_exp_rat": std_100_exp_rat})
